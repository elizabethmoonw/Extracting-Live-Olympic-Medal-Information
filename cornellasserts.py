...
def _close_descent(a, b, rtol, atol, equal_nan, flatten=True):
    ...
    if flatten:
        ...
        for pos in range(len(a)):
            result = result and _close_descent(
                a[pos], b[pos], rtol, atol, equal_nan, flatten)
    else:
        result = []
        for pos in range(len(a)):
            result.append(_close_descent(
                a[pos], b[pos], rtol, atol, equal_nan, flatten))
...
def assert_equals(expected, received, message=None):
    ...
    if (expected != received):
        if message is None:
            message = 'assert_equals: expected %s but instead got %s' % (
...
def assert_not_equals(expected, received, message=None):
    ...
    if (expected == received):
        if message is None:
            message = 'assert_not_equals: expected something different from %s' % repr(
                expected)
        quit_with_error(message)
...
def assert_true(received, message=None):
    ...
    if (not received):
        if message is None:
            message = 'assert_true: expected True but %s evaluates to False' % repr(
                received)
        quit_with_error(message)
...
def assert_false(received, message=None):
    ...
    if (received):
        if message is None:
            message = 'assert_false: expected False but %s evaluates to True' % repr(
                received)
        quit_with_error(message)
...
def assert_floats_equal(expected, received, message=None):
    ...
    if type(expected) not in number:
        if message is None:
            message = (
                'assert_floats_equal: first argument %s is not a number' % repr(expected))
    elif type(received) not in number:
        if message is None:
            message = (
                'assert_floats_equal: second argument %s is not a number' % repr(received))
    elif (not isclose(expected, received)):
        if message is None:
            message = 'assert_floats_equal: expected %s but instead got %s' % (
                repr(expected), repr(received))
    else:
        message = None
...
def assert_floats_not_equal(expected, received, message=None):
    ...
    **IMPORTANT**:
    ...
    if type(expected) not in number:
        if message is None:
            message = (
                'assert_floats_not_equal: first argument %s is not a number' % repr(expected))
    elif type(received) not in number:
        if message is None:
            message = (
                'assert_floats_not_equal: second argument %s is not a number' % repr(received))
    elif (isclose(expected, received)):
        if message is None:
            message = (
                'assert_floats_not_equal: expected something different from %s' % repr(expected))
    else:
        message = None
...
def assert_float_lists_equal(expected, received, message=None):
    ...
    **IMPORTANT**: The arguments expected and received should each lists of
    ...
    if not type(expected) in [list, tuple]:
        if message is None:
            message = (
                'assert_float_lists_equal: first argument %s is not a sequence' % repr(expected))
    elif not type(received) in [list, tuple]:
        if message is None:
            message = (
                'assert_float_lists_equal: second argument %s is not a sequence' % repr(received))
    elif not _check_nested_floats(expected):
        if message is None:
            message = (
                'assert_float_lists_equal: first argument %s has non-numeric values' % repr(expected))
    elif not _check_nested_floats(received):
        if message is None:
            message = (
                'assert_float_lists_equal: second argument %s has non-numeric values' % repr(received))
    ...
    try:
        if not allclose(expected, received):
            error = True
            if message is None:
                message = 'assert_float_lists_equal: expected %s but instead got %s' % (
                    repr(expected), repr(received))
    except Exception as e:
        error = True
        if message is None:
            message = 'assert_float_lists_equal: sequences %s and %s are not comparable' % (
                repr(expected), repr(received))
...
message.  For example::
    ...
    if not type(expected) in [list, tuple]:
        if message is None:
            message = (
                'assert_float_lists_not_equal: first argument %s is not a sequence' % repr(expected))
    elif not type(received) in [list, tuple]:
        if message is None:
            message = (
                'assert_float_lists_not_equal: second argument %s is not a sequence' % repr(received))
    elif not _check_nested_floats(expected):
        if message is None:
            message = (
                'assert_float_lists_not_equal: first argument %s has non-numeric values' % repr(expected))
    elif not _check_nested_floats(received):
        if message is None:
            message = (
                'assert_float_lists_not_equal: second argument %s has non-numeric values' % repr(received))
    elif len(expected) != len(received):
        return
    else:
        error = False
    ...
    try:
        if allclose(expected, received):
            error = True
            if message is None:
                message = 'assert_float_lists_not_equal: expected something different from %s' % repr(
                    expected)
    except ValueError:
        pass
    except Exception as e:
        error = True
        if message is None:
            message = 'assert_float_lists_not_equal: sequences %s and %s are not comparable' % (
                repr(expected), repr(received))
    ...
    if not callable(func):
        ...
        try:
            ...
            if message is None:
                ...
                message = ('assert_error: call %s%s did not crash but instead returned %s' % (
                    func.__name__, body, repr(result)))
                ...
                if type(reason) == tuple:
                    if reason != e.args:
                        ...
                        if message is None:
                            ...
                            message = ('assert_error: %s has reason %s, not %s' % (
                                name, repr(e.args), repr(reason)))
                    ...
                    if len(e.args) == 0 or reason != e.args[0]:
                        ...
                        if message is None:
                            name = e.__class__.__name__
                            if len(e.args) == 0:
                                message = ('assert_error: %s has no reason, but expected %s' % (
                                    name, repr(reason)))
                            else:
                                payload = e.args[0] if len(
                                    e.args) == 1 else e.args
                                message = ('assert_error: %s has reason %s, not %s' % (
                                    name, repr(payload), repr(reason)))
            elif message is None:
                name1 = e.__class__.__name__
                name2 = error.__name__
                body = repr(args) if len(args) != 1 else '(%s)' % repr(args[0])
                message = ('assert_error: call %s%s crashed with %s, not %s' % (
                    func.__name__, body, name1, name2))
